...
public enum SingletonApplicationContext {
	...
	public static synchronized ApplicationContext get(Map stormConf, TopologyContext topologyContext) {
		...
		if (entry == null) {
			...
			entry = instantiate(properties, topologyContext);
	...
	private static ApplicationContext instantiate(Map<String, Object> stormConf, TopologyContext topologyContext) {
		String[] configLocations = {format("classpath:/%s-context.xml", topologyContext.getStormId())};
		AbstractApplicationContext result = new ClassPathXmlApplicationContext(configLocations, false);
		result.setId(configLocations[0]);

		if (stormConf != null && ! stormConf.isEmpty()) {
